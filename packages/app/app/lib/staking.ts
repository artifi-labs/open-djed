...
export function expectedStakingReturn(
  ...
  while (creditDate <= end) {
    const periodDays = (creditDate.getTime() - lastCreditDate.getTime()) / DAY_MS
  ...
  const pendingReward =
    includePending && pendingDays > 0 ? calculateReward(currentBalance, aprPercent, pendingDays) : 0
  ...
  const totalCreditedRewards = credits.filter((c) => c.credited).reduce((sum, c) => sum + c.reward, 0)

  const totalPendingRewards = credits.filter((c) => !c.credited).reduce((sum, c) => sum + c.reward, 0)
...
function calculateReward(balance: number, aprPercent: number, days: number): number {
