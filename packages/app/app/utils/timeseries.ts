...
export function timeBucketFromStart(date: Date, intervalMs: number, start: Date): Date {
...
export function aggregateByBucket(
  ...
  if (data.length > 0) {
    const firstAgg = convertRawToAggregated(data[0], aggregations)
    const lastAgg = convertRawToAggregated(data[data.length - 1], aggregations)

    const filtered = result.filter((r) => r.date !== firstAgg.date && r.date !== lastAgg.date)

    return [firstAgg, ...filtered, lastAgg]
  }

  return result
}

function convertRawToAggregated(row: DataRow, aggregations: AggregationConfig): DataRow {
  ...
  for (const col in aggregations) {
    ...
    if (methods.includes('sum')) out[`${col}_sum`] = value
    if (methods.includes('avg')) out[`${col}_avg`] = value
    if (methods.includes('min')) out[`${col}_min`] = value
    if (methods.includes('max')) out[`${col}_max`] = value
  }
